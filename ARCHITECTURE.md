# AMA-teras アーキテクチャ設計(旧称 MyCodex)
...
- **スモークテスト**: `AMATERAS_SMOKE=1 electron .` で BrowserWindow を開かず
...
1. **B環境作成**(worktree.ts):
   `git worktree add ../amateras-evolve/job-<N> -b evolve/job-<N>`(リポジトリ外の兄弟ディレクトリ)。
...
3. **検証ゲート**(gates.ts、全合格が昇格条件。B内で子プロセス実行):
   ...
   2. `npm run typecheck`
   3. `npx vitest run`
   4. スモーク: B内で `npm run build` 後、`AMATERAS_SMOKE=1 electron . --tool <name> --input <sample.json>` を実行し exit 0 を確認
4. **昇格**(promote.ts):
...
- **スモーク**: `AMATERAS_SMOKE=1` 起動パス自体が進化ゲートで常時使われるため実質E2E
