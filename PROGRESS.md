...
| 5a 耳(PTT) | ✅ 実装・テスト | ⏳ **未実施**(whisper 未配置) |
| 5b 耳(常時) | ✅ 実装・テスト | ⏳ **未実施**(whisper 未配置・30分連続稼働の確認も) |
| 6 PC窓観測 | ✅ 実装・テスト | ⏳ 未実施(運営モードONが必要) |
...
テスト: vad.test.ts(5)。月読95テスト緑。

**未実施**: 30分連続稼働のメモリ・CPU確認(whisper 配置後)。
