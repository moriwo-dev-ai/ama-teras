...
- (ユーザー)M28の確認事項4点(冒頭「未解決・ユーザーへの連絡事項」参照):
  新UI一目確認 / レジストリ実疎通 / GitHub作成・push・公開(すべてユーザー作業)/
  bash制約の把握
- (ユーザー)M27からの持ち越し: 無料API実疎通(接続テスト)/ Roaming\Electron削除
  ※Author名義はM27-7→再書き換えで解決済み、戦略文書は公開前の最終目視のみ残
