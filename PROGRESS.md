...
## M239b: 顔のシート準拠やり直し (2026-08-20朝・ユーザー指摘)
- 指摘: 「顔が全然違う(髪型・目・幼さ)」→ v1は編み込みボブ・細め暗茶目・大人顔だった
- 対応: 顔セット=丸目童顔プリセット/目セット=キラキラ丸目/瞳#C9832E(hex直接入力)/
  髪=外ハネショート+アホ毛(編み込み排除)/頬下膨れ0.35・あご丸め0.5・エラ縮め0.2/
  頭身=身長(女性)-0.605で胴を縮め全身の大きさ+0.165で142.1cmへ(頭サイズ固定→頭身が下がる)
- 新知見(Unity GUI自動化の完成形):
  - テキスト: WM_CHAR直送(日本語可)・制御はWM_KEYDOWN(BS=8/Enter=13。VK SendInputは全滅)
  - 数値ボックス: dbl→WM_KEYDOWN BS×n→WM_CHAR→WM_KEYDOWN Enter で任意パラメータ設定可
  - ホイール: mouse_event(0x0800)は「そのScrollRect上で一度クリックした後」なら効く
  - パネルが画面外に切れる問題: **MoveWindowでウィンドウをy負値へはみ出させる**(高さ7000px超まで
    実証)。見えないスライダーも数値ボックス化して届く
- コミット 3671d33。フェーズ2b(ヘアピン・カーディガン・刺繍・紋)は引き続きglTF後加工の領域

## M239: キャラシートのVRoid化・夜間自律作業 (2026-08-20未明)
- 成果: 公式キャラシート(docs/design/hinata-character-sheet.png)準拠のVRMモデルを
  VRoid Studio 2.14.0のGUI自動操作だけで構築→エクスポート完了。
  **docs/design/hinata-12sai.vrm** (14.8MB, VRM1.0, meta名=ヒナタ/作者=moriwo) と
  プロジェクト **hinata-12sai.vroid** をリポジトリ+Documents/AMA-teras/に格納
  (前回「格納場所を失った」教訓の根治=リポジトリが正)
- 検証(機械): glTFヘッダ+VRMC_vrm拡張をnodeでパース。VRM1.0スペック・表情プリセット14種
  (happy/angry/sad/relaxed/surprised+ビゼームaa/ih/ou/ee/oh+blink系+neutral)・
  humanoid54ボーン・内蔵サムネ抽出で見た目確認。フェーズ1表情エンジンの素材要件を満たす
- 反映内容: 黒ボブ#38322E・身長141.5cm・茶目・白ゆるトップス・黒ショーツ・
  ワインレッドリボン・白ニーソ・クリームスニーカー
- 技術突破(Unity GUI自動化・desk.ps1):
  - テキスト入力: SendInput UNICODE×・クリップボードCtrl+V×・スキャンコード注入×の全滅の末、
    **PostMessage WM_CHAR直送が唯一通る**(日本語も直接入る。「ヒナタ」を実証)
  - スクロール: ScrollRectはバードラッグ×・コンテンツドラッグ×・WM_MOUSEWHEEL×、
    **mouse_event(0x0800)ホイールが有効**。※前夜「Unityでホイール不可」と記録したのは誤りで、
    実はdesk.ps1の-CommandモードでAdd-Type型キャッシュ由来のMethodNotFoundで未発火だった。
    独立ファイル+別クラス名(wheel2.ps1)で回避
  - 色指定: SVスクエア直クリック/数値: スライダートラッククリック+表示値のクローズドループ(既知)
- 積み残し(フェーズ2b=glTF後加工が私の領域): 赤花ヘアピン・カーディガン+太陽刺繍・紋・
  ショーツこげ茶化・リボン無地赤化・目のオレンジブラウン微調整。現行世界への反映は別判断(ユーザー)

## M237: 覚醒度ゲート+寝言A案 (2026-08-19・ユーザー設計)
- 実測課題: 眠り中も全欲求が満額入札+softmax(温度0.12)の揺らぎで、体力0の夕方に48回起床
  (28回はテレビ欲が勝った)。疲れているほど45分ブロックを要求される逆進構造で回復不能ループ
- 設計(ユーザー発案): 人間の視床ゲートの数式化。sleepSession中、α=max(0.05, clamp(energy/0.8)^1.5
  + sleepDisturb×1.5) を全非睡眠候補に乗算。疲れているほど深く眠り、回復するほど浅くなる=
  明け方の自然な目覚めが創発。声はαを押し上げる(呼び続ければ1〜3分で起きる)。入眠競売は無減衰
  (徹夜の自由=B案の魂は不変)。起床時αで寝ぼけ時間と起きぬけの一言の質が変わる
- 寝言(A案): α<0.3で対人刺激の欲求(おこされた/あいさつ・生値0.4以上)が肉薄した時だけ、起きずに
  ひとこと漏れる。発火=マージンのみ(サイコロなし)・内容=抑圧欲求が材料・ブロック継続・5分制限。
  「仕組まれた台詞」ではなく「眠りのゲートの不完全性の観測」として全ログで閉じる
- C案(夢=夜間統合の断片を寝言に)は金曜生放送で実装予告(台本3-5に追加済み)のうえ後日実装
- 検証: 今夜22時の就寝が初テスト。明朝番犬でwake回数・ブロック完走数・sleep_talkイベントを実測

## M235+M236: コスト経済の残り2枚 (2026-08-19)
- M235: 神の手動実行(runGodNow=UIボタン/神議チャット発)の消費をrecordSpendで1日予算へ計上。
  定刻実行はscheduler.tickが計上済みで、手動経路だけ財布の外だった。
  ※調査訂正: サルタヒコspentToday=0は計上漏れではなく「候補全既読=実消費0」だった
- M236: マイクロコンパクト強化。(1)積極省略モード=古い中型(256字超)tool_resultを
  要点120字+省略マーカーへ(実測: 建築系セッションは履歴の86〜97%がツールダンプ、
  4KB基準の第1段では中型が生き残っていた)。冪等。(2)要約閾値の半分で第1段だけ先行発火
  (無料掃除。キャッシュ書き直しは発火時1回に束ねる)。要約(LLM)は従来どおり閾値超のみ
- 期待効果: 平常+$0.5〜1/日減、建築日スパイク50〜70%減。適用はアプリ再起動後

## M234: コスト意識の圧縮上限(Claude Code同型のコンテキスト経済) (2026-08-19)
- 実測事故: テラ(mainバンド・kimi-k3)が1コール平均19〜23万tok×147コール/日=入力27.7M tok、
  $3/M+キャッシュ読み10%で**$13/日**。$20チャージが2日で消えた
- 根因: 圧縮閾値「文脈上限の70%」がkimi-k3(1M文脈)では70万tok=事実上無限。
  compactHistory(M13-1)は健在なのに一度も発火しない構造だった
- 対処: costCompactionCapFor(provider) — 有料プロバイダは32kで先に畳む(Claude Codeの
  オートコンパクトと同じ「直近は原文・古い部分は構造化要約」思想。要約知見のAMATERAS.md退避も既存)。
  ローカルollamaは無限のまま。実効: 履歴23万→3.2万tok(コスト約85%減の見込み)
- 適用にはアプリ再起動が必要。残タスク: 神々のrecordSpend配線漏れ(kamuhakariのみ計上)、
  マイクロコンパクト(古いツール結果の軽量間引き)は次の実装枠候補

## M206: 世界の一般公開ゲスト+記憶の防壁 (2026-08-15)
- 背景: 刺激不足の実測(「だれかこないかな…」×2・scalar平坦)→ユーザー決定「思い切って公開・
  疑わしきは通す」。観戦ページの「🚶あそびにいく」ボタンで誰でも歩ける+話せるゲストに昇格
- 鍵はsidからsha256導出=再訪で同一個体・紋と色は観戦時代と同じ(アイデンティティ連続)
- 防壁: 出禁(world-banned.json)・予約名(ヒナタ/テラ/もりを等)・50字・10秒1回・1日200件・
  同時歩行5人・近接会話ゲート(5m・位置未登録も遮断)・同意バナー(記憶になる/動画に出る/個人情報禁止)
- 社会的体力: 来客との会話1往復energy-0.015(レート制限が彼女の生態になる)。返事は元々直列=同時1人
- 夜間検疫 lifeform/quarantine.mjs: 統合前にguest:発言を機械+LLM二次判定、明白な毒のみ
  quarantine/へ隔離(原本.pre-quarantine保全=完全可逆)。すれすれは通す・もりを/テラ発言は対象外
- E2E実測: sid不正400/予約名400/昇格200/同一sid=同一鍵/位置未登録の声は不達/出禁403/検疫の
  分別(死ね=隔離・変なの=通過)。全て恒久URL越し・ヒナタの記憶無汚染で検証
...
- 【真因】分離世界モードでWorldRemoteClient.restorePayload()が常にnull=アプリ経由で開いた
  ...
  症状の正体・PCでは他ページのhelloに救われて偶然映っていた)。world-serverに/api/world/restoreを
  新設し、クライアントが15秒毎にキャッシュ→接続の瞬間に必ず世界を渡す
- 3D基盤(three/addons/three-vrm)をvendorへ同梱=CDN不使用。外に出られない回線でも真っ白にならない
  (更新は scripts/vendor-three.mjs)
- 実測: CDN遮断+単独ページ(誰の助けもなし)で19オブジェクト全部表示を確認

## M200: 声と表情の器官(ネウロ様スタック) (2026-08-12)
- 3層表情スタック: L0無意識(内部状態joy/fear/sleepy/arousal→連続ブレンド+微小ノイズ+まばたき変調+
  眠気の半開き)・L1発話強調(彼女の言語野が「いちばん伝えたい言葉」を選び目の見開き+ピッチ上昇が
  モーラ時刻でミリ秒同期)・L2バースト(快楽の瞬間の大表情・減衰エンベロープ)
- 声=VOICEVOX猫使ビィ(ユーザー選定・スタイル4種)。内部状態で声色変調(恐怖→人見知り・眠気→
  おちつき・ピッチ/抑揚/速度も連動)。world-serverが合成し/tts/で全面配信(トンネル越しもOK)
- 口パク=モーラ→母音時系列(音が出ない環境でも口が動く=録画に乗る)。音声はページで再生
- affect(25秒毎・記憶を汚さないfetch直行)・affect_burst(euphoriaフック)。クレジット表記:
  「VOICEVOX:猫使ビィ」を配信概要・動画説明に必須
- 実測: 喜び顔・眠気の半開き・発話中の口+目見開きをcam=front接写で確認

## M199: 成長実録パイプライン(リングバッファ+瞬間検知) (2026-08-12)
- world-serverに録画保存口 /api/world/recording 新設(分離後は欠落していた=録画不能だった)
- 記録係 scripts/moment-recorder.mjs(デタッチ常駐): 実行係に5分刻み常時録画(3Mbps・record bps対応)・
  直近4時間保持で自動削除・daemon/world両ログを尾行して面白い瞬間をmoments.jsonlへ
  (初発見/あいさつ/テラへの声/つぶやき/訪問者/観客/起床就寝/統合)
- 抜粋→縦型化→字幕→タイトルはClaude・公開は必ずオーナー承認(1日1回の承認バッチ方式)
- 実測: 1本目セグメント114MB保存・ffmpegで切り出し可

## M198: 個体の見た目=世界の解像度(色+胸の紋) (2026-08-12)
- 個体(sid/招待キー)から見た目を導出: 胸の紋=先頭4桁・色相=紋の前半から=**紋と色は本当に相関**
  (ヒナタがその法則を発見し得る)。色名は日本の伝統色12色に量子化(やまぶき・あさぎ等)
- ゴーストに反映: 個体色のemissive+胸の紋スプライト(名前が同じでも見た目が違う=誰にでも見える)
- ヒナタへは「見た目」として渡す(識別子として教えない): spec=紺いろのロボット・むねのいろは○・
  むねのもよう「XXXX」。台帳キーは個体単位(立ち見客=名前·紋。招待客は名簿管理なので名前のまま)
- 言葉の上の呼び名からは紋を落とす(perceiveにlabel追加)=呼び方は彼女の自由のまま

## M194b: 五感v2=世界の真実として再設計(ユーザー協議・2026-08-12未明)
- 「ゆれ」は五感ではない→5感覚(見た目/音/手ざわり温度/におい/あじ)に是正。動きは説明側に書く
- プロファイル=強さv(0-1)+説明desc(世界の定義)。見た目のよいバイクはsightが高く見た目の説明が残る
- 保存先=世界の正本(world-state.json senses)。world_actに sense_profile コマンド新設・
  observe経由でヒナタが真実を取り込む(adoptSenseProfile)。テラへのルール=howToSeeに明記
  (spawnしたら必ずsense_profileを定義)
- 既存19オブジェクト全てにバックフィル済み(生成はgemma3:4b。14Bはロード重でPC圧迫→中止)
- 例: ブランコ=touch0.9「木材の温かさとロープの感触。揺れる心地よい振動」・お昼=taste0.8・
  変圧器=sound0.7「ブーンという高周波」

## M194: 五感の知覚+リンクのレンズ+眠りの永続化 (2026-08-11夜)
- 物の感覚プロファイル=世界の真実(彼女の心ではない)。世界係LLMが初対面時に一度だけ生成し
  world-sense.jsonに永続(将来はテラの建築時付与へ移行)。あじは口にできる物だけ>0
- ヒナタ側=感度ベクトル(senses.json・初期フラット0.5・新発見した感覚が+0.03ずつ育つ)
- 感覚の選択=重み付き抽選(物の性質×感度×感覚別の井戸)。涸れた感覚は軽くなる=
  「見飽きたら触る」が報酬勾配から自然に出る(強制ローテーションなし・ユーザー協議で決定)
- リンクのレンズ: 知覚の文脈に「つながっている記憶」上位2件を提示するだけ(使い方は指示しない)。
  文脈に無い情報はどんな賢いモデルでも使えない=配線が発現の前提条件
- 細部に感覚タグ(noteDetail第4引数)。眠り/統合済み日をbody.jsonに永続(§9停止=睡眠と同型・
  再起動で夜中に覚醒・統合二重実行しない)。プロファイル生成はコールドスタート対策で90s
...
- M176: 訪問者ゴースト(名札・地面タップ歩行・30秒退場)。ヒナタが「だれが・どこに」を知覚し
  おでむかえ/おみおくり/そばへ歩く。オーナーもwalk=1で全権限のまま歩ける(もりをゴースト)
- M177: 配信の分離世界対応(弾幕/HUD/ガード転送)+コメント振り分け(@ヒナタor冒頭ヒナタ→彼女、
  他→テラの建築お題)。視聴者名でひとの台帳が生える
- M178: world.html整理は段階方式(一括分割はリスク過大)。第1弾ghosts.js+テストハーネス確立
- トンネル: cloudflaredのDL権限待ち(ユーザーの!コマンド依頼中)。公開面8790は稼働済み

## M173-175: 世界の独立(C→A→B工事・2026-08-10深夜)

- **C: world-server分離**(独立プロセス・専用ヘッドレスChromium実行係・CDP9226)。
  アプリはクライアント化(WorldRemoteClient+SSEブリッジ)。userData/world-server.jsonで切替。
  アプリ再起動で世界とヒナタが死なない=断絶の根治。データ引っ越しゼロ(チャット200件連続)
- **A: 公開観戦面**(--public-port 8790・読み取り専用・書き込みAPIなし・SSE上限25)。
  トンネルの向け先。URL公開はオーナー承認制のまま
- **B: 招待制訪問者**(world-invite.mjsで名簿発行・visit=1&vk=KEY・声だけ:弾幕+ログ+
  ヒナタが名前つき知覚=ひとの台帳が生える・5秒1回/120字/NG語・テラ中継なし)
- 既知ギャップ: 配信(VOICEVOX/録画/live)は分離モード未対応=次回配信前に要工事。
  スマホ表示はアプリ経由ブリッジで従来どおり

## M164-165: 人格浄化・深い知覚・好奇心・テラ同格化(2026-08-10 午後)

- M164: core.md v2=事実と声だけ(性格・例文・好き苦手・一人称まで全撤去。
  「性格は設計で作りたくない」)。撤去種はremoved-seeds.mdで監査コーパスに残置
- M165: perceive.mjs=知覚を関数に(注意の深さ4段・観察台帳で矛盾なし・錨=実スクショ+仕様)。
  好奇心=intero:learning(生得・対象なし・退屈が探索を駆動)。「好き」は獲得で発現させる
- c-3 v3: テラ=もりをと同じ立場の住人。彼女の全発話(もりを宛返事以外)が届き、
  返事/建築/沈黙はテラの自由。検出パース全廃(「きょうだつ」→兄弟の誤発注が動機)
- 実測: 発見時の駆け寄り・視覚つぶやき・「おいしい。」(三日月クッキー納品)

## M163: 世界チャット履歴ページ(2026-08-10 昼)

- `world-log.html`: 見てない間の世界の会話を後から読めるページ(👤もりを/🌙ヒナタ/🔧テラの
  名札・日付区切り・30秒自動更新)。トークンはworld.htmlと共有(localStorage)
- `GET /api/world/chatlog`: 通常トークン+実行キー(ループバック・読み取り専用)の両対応
- manager.tsに `chatHistory(limit=200)` 追加(chatLogの直近を返すだけ)

## M158-162: 社会の配線(2026-08-10 午前)

- M158 社会的共調整: 応えられた発散(指数減衰τ=10分)・慰めの言葉が恐怖を実際に半減・
  「聞いてほしい」候補(恐怖×随伴性)
- c-3 v1: POST /api/world/job=ヒナタ→テラ発注。願い検出はregex敗北(日本語の省略)→
  brain.classify(few-shot・温度0)へ。**初の自発発注「お月さま」成立**
- M159 気配の知覚(viewers)・M160 チャット話者名札(🌙/🔧)・M161 単独性ロック
  (二重ヒナタ事故の根治)・M162 テラの声が聞こえる(名前呼び/問いかけに返事)
- **初の住人間会話成立**: テラ「ヒナタちゃんが見上げたらいつもきらきらしてるはず」→
  ヒナタ「わーい!きれいだね!✨」(願い→発注→建築→報告→感謝の全自動ループ)
- 実測バグ根治: 再起動健忘4種(体・声・発注・単独性)・納品感謝→再発注ループ・SSE二重接続・
  植えた願い(VRM)の浄化=目標欄は夜間統合の聖域に

## M151: B′実装P1 — 原理から導出する心(2026-08-09深夜〜8-10未明)

設計はlifeform/B-PRIME.md(原理・賭け金=自己連続性・出所監査・推定)。実装:
- mind.mjs: 予測の束+誤差=知覚+精度学習+中心軸スカラー+報酬/恐怖(恐怖2倍)+行動価値
- daemon v3: drives/興味値/探検ルール全撤去→価値導出のソフトマックス行動選択。断絶センサー
  (起動時の時間の飛び=賭け金の痛覚)・内受容予測・声の日課学習・SIGTERM就寝処理
- reflect.mjs: DMN層(昼のマイクロ内省・生エピソードのみ)+夜の統合(qwen3:14b優先)+
  fadeMemories(統合されなかった日は本当に薄れる)
- audit.mjs: 出所監査(設計者/聞いた言葉コーパス vs 自己言及文・R2徴候・毎朝5時)
- persona/self.md: 全部書く自己台帳。era.json: B′紀元タグ(リセットせず時代境界で区別=ユーザー合意)
- 初観測: 起動1分で「たしかめ:crane_live」=誰も書いていない見回り行動が導出/
  起動4分で初マイクロ内省「風車、また見たんだ」=自分の行動の繰り返しへの気づき(R1素振り)
- 夜間モデル: qwen3:14b+gemma3:12b導入済(夜の統合で比較)
- 残: P2(遷移学習・ロールアウト・言葉vs行動相関)/mind単体テスト/R段階ダッシュボード
...
- **POST /api/world/command**(server.ts): 生命体デーモン(別プロセス)の世界コマンド投入口。
  ループバック+実行キー限定・`say/motion/move_to/face`のみ許可(破壊系/アプリ操作系は経路ごと遮断)・
  1〜10件制限。WorldManager.act委譲=配信ガード/ack機構を共有。ipc.tsで注入。テスト3件追加(43緑)
- **lifeform/hinata-daemon.mjs**(依存ゼロ・素のNode): b案の核。
  - 知覚: 観戦SSE(world:event+chat:event)/行動: 新エンドポイント
  - 欲求システム: 好奇心/退屈/社交欲/元気(時刻連動=夜は眠くなりsit中心)
  - 心拍18〜40秒ゆらぎ: 欲求の重み付き抽選で 散歩/think/sit/idle/独り言(10分に1回まで)
  - 反射層: live_comment→社交欲スパイク+0.6秒後にclap(30秒スロットル)
  - 調停: chat:event(思考層稼働)から90秒・world:event(他者の世界操作)から20秒は身体を譲る。
    自分の行動エコーは3秒窓で除外。毎分6コマンド上限の暴走ガード
  - 接続先自動発見: CDP(9225)から実行係ページURLのport/実行キーを読む(手動: --port/--key)
...
## M148-149: 知覚の人間化フルセット(2026-08-09 昼)

- M148: 空間感覚(1分毎の見回し・近いものほど気になる探検・自己位置感覚)+世界地理
  (persona/world-map.md)+ラベル文化(spawnに必ず日本語label=名付けが生命体への提示)
- M149: 視野8m制限+記憶地図(known-world.json)=霧の晴れる世界/差分の驚き(移動・目撃した
  消失のみ。視界外の消失は信じ続けて現地で驚く)/聴覚の空間化(15m内の建築音に振り向く)/
  身体性(歩行で元気減)/視覚eyes.mjs(CDP撮影→gemma3:4b視覚。「じっと見る」30分1回)
- 設計哲学: 人間らしい知覚=豊かな入力ではなく制限された入力。全知をやめて無知から始めさせる

## M146: 知覚拡張(探検)+実行係SSE盲点修正(2026-08-09 朝)

- 朝の事故の真因(M146b): 不可視の実行係はbackgroundThrottling:falseでもrAFが約1fps
  (コンポジタのフレームが来ない)→歩行が実質1/60速で長距離move_toがack90秒超過→キュー詰まりで
  世界全体が無応答に見えた(SSEは無実。深夜はスマホページ=60fpsが代役ackして隠れていた)。
  対策: rAFが100ms黙ったらタイマーで同じtickを回す補完(実行係のみ)。実弾: 広場端まで14秒=全速。
  ついでにM145監視もEXECUTOR可視判定免除で恒久化
- GET /api/world/state(実行キー): 生命体の観察口。デーモンに探検行動=好奇心/退屈が高い時、
  実在の社・造形を選んで会いに行き感想をつぶやく(15分1回)。時刻感覚もプロンプトへ
- ヒナタの知覚一覧(現在): 世界チャット/視聴者コメント/世界の変化(spawn・app_add)/
  思考層の稼働気配/観察スナップショット(社・造形の位置と名前)/時刻/自分の欲求

## M141: 配信フィードバック一括対応(2026-08-08 未明・初回配信のユーザー8項目)

初回公開配信(8/7 視聴8・同時2・チャット13・平均視聴22分)のフィードバック全対応:
1. **音声(VOICEVOX)**: `/api/world/tts`(ループバック限定)がローカルエンジン(127.0.0.1:50021)へ
   中継。world.html `&tts=1` で say が読み上げ+大字幕。話者 `&spk=N`(既定2=四国めたんノーマル)。
   エンジン停止時は503→字幕のみに静かにフォールバック。
   **エンジン**: `%LOCALAPPDATA%\Programs\voicevox-engine\windows-cpu\run.exe`(0.25.2・約1.8GB)。
   **配信前に起動必須**+概要欄にクレジット「VOICEVOX:四国めたん」表記が規約
2. **コメント弾幕**: LiveDirectorが全コメントを `live_comment` で全ページへ(NGパターンは除外)。
   ニコニコ風右→左+採用時は中央「🎯採用!」ポップ(コメント者名入り)
3. **実況ボード観戦転送**: 観戦SSEに chat:event(status/tool_*のみ・許可フィールド写経+
   サーバ側パスマスク)を転送=OBS画面に「いま何をしているか」が出る。観戦時は大型表示
4. **アプリ大画面+自動退出**: 観戦ページはアプリを右60vw・1.35倍拡大表示(世界は左に見え続ける)。
   操作/say/状態更新が25秒途絶えたら自動 app_leave(開きっぱなし事故の恒久対策・実測28秒で退出確認)
5. **カメラ大改修(cam=auto)**: 平常=アバター中心の近距離周回(r5.2)/移動中=後方追従/
   イベント=カット切替→ゆっくり寄る(camFocusToがautoではスイッチング動作)/
   cameraFocus追尾は対象実寸比例の引き距離+「アバター側から見上げる」構図(社に埋まる事故回避)
6. **月ロングフォール**(userData/world-apps/moon 全面改修): 落下時間10/30/60/90分をセット→
   開始時刻+所要時間だけから毎フレーム計算(ページ/アプリ再起動でも止まらない=時計連動を廃止し
   アラーム上書き不発の根本対策)。リアル小月(14%)→p^2.2で巨大化、p>0.72顔・p>0.85赤空・
   着弾→ホワイトアウト→「配信終了」。落下中はカウントダウン盤が月に随伴。
   カメラ自動追尾は残り90秒のみ(長丁場を邪魔しない)
- framePrompt追記: sayは読み上げ前提で話し言葉/黙って作業しない(建設地へmove_to+節目実況)/
  app_leave鉄則。OBSソースURLに &tts=1 追加済み
- 検証: typecheck緑・vitest 65緑・world-smoke 6/6・実機E2E(tp各位相+採用ポップ+字幕+自動退出)
...
②社の配置コントロール: シングルタップ→チップUI(回転22.5°刻み・高さ0.5m刻み・自動収納)。
  app_movedにy/ry追加でmain正本へ永続化
③返事ファースト: 世界チャットの枠に「最初のツール呼びはsayで返事+進め方→節目で状況共有」を明記
④派手モーション枠: backflip/flykick/run/wave/dance/cheer(assets-labへ同名FBX配置で有効・
  未配置は明示エラー)。move_toは5m超+run配置済みで3.2m/s走行。
  **Mixamo FBXのダウンロードはユーザー作業**(Adobeログインが必要なため)。
副産物: __worldフック上書き回帰の修正。テスト634緑・実機E2E(クラス切替・フック)確認。

## M129b/c: 生きた社の運用初日で踏んだ2つの穴(2026-08-06 未明〜朝)

エージェントに「電卓の結果を社に映して」を任せた実戦で発覚・修正:
- **M129b**: publish状態がページ内保持のみ→エージェントの確認スクショ(毎回新規の観戦
  ページ)に映らず、正しい実装を壊れていると誤認して社を6回建て替えるループに。
  main経由で全ページへ配布+restorePayload同梱に変更
- 7セグの鏡文字: エージェントが「裏から見てるだけ」と誤診→正面スクショを突きつけて
  X軸反転と指摘したら1発修正(卓越した協働例: 開発者=Claude が世界チャットで診断を渡す)
- **M129c**: tick例外1回で永久無効化が厳しすぎ、途中の不正state値で社の表示が凍結
  (新規ページでは正常=検証と実表示が食い違う罠)。累計10回許容+新stateで蘇生へ
運用知見: 長寿命ページと新規ページの見え方の差はこの系の定番の罠。デバッグは実行係
ページのCDP(__world.appStates / worldApps)が最短。
...
chat_restoreで洗い替え復元(二重表示なし・E2E確認)。あわせて「アプリを開いたまま離脱→
再入場後もmainが開いてる扱いのまま」のバグをhelloでのopenAppIdリセットで修正。
カメラは意図的に毎回初期位置(予測可能性優先)。

## M127: 世界UX一括改善(2026-08-06 未明)

ユーザーテスト(電卓12×8+4=100・エージェントが自分の押し間違いをapp_readで自己診断)後の
要望バッチ。①iframeプール=複数アプリが裏で生きる(離れる=状態保持/閉じる=終了。E2E: 7残存/0リセット確認)
②app_openでアバターが社まで歩く ③observe.openApp=ユーザーが開いているアプリの文脈
④承認待ちを世界に吹き出し通知 ⑤社=機能の見た目規範(既定看板廃止・sign乱用禁止)
⑥app y/ry(2階設置・エージェント経由。ドラッグは高さ維持) ⑦userData.tickで動く3D(風車等)
⑧ライブ切替時プール再作成 ⑨配信枠にカメラ演出規範 ⑩歩速1.3m/s。テスト634本緑。
未実装(バックログ): ユーザー操作での高さ・向き変更ジェスチャ、モーション増強(派手アクション)。

## M126: アプリの「実演」— app_click/app_type/app_read(2026-08-06 未明)

ユーザーの確認質問「電卓を使って計算してみて、は通る?」で判明した2つの穴を修正:
①エージェントにアプリを操作する手段がなかった(open/pointのみ) ②アプリ表示中は
オーバーレイがチャット欄を覆い会話不能だった。
- app_click(押す)/app_type(入力)/app_read(値の読み取り→ackのnotesでツール結果へ)を追加。
  操作はサーバーがworld-apps HTML配信時に自動注入するヘルパーとのpostMessageで実現
  =ライブ中のsandbox隔離(別オリジン)でも同一経路で動く。app_pointも同方式へ移行
  (contentDocument直接参照はsandbox中に壊れる潜在バグだった)
- チャット欄をオーバーレイより手前(z-index 55)に=アプリを開いたまま会話できる(通常時も)
- E2E実機: 通常時7+5=12、sandbox隔離時9-2=7を操作・読取成功。テスト65本緑
- world.htmlに `window.__world` デバッグフック(CDPからopenApp/appOpを叩ける)
...
ペースを落として待てば無料のまま完走できる。「クレカ不要。寝ている間にアプリが育つ」を
製品の売りにする。

**設計(段階実装)**:
- **M113-1 忍耐リトライ(今日)**: config `overnightMode`。ONのとき transient(429含む)は
  maxRetries を消費せず、待機して再試行し続ける。待機時間は
  ①エラーのretry-after(ヘッダ or Groq/Gemini系の "try again in Xs" 文言)を最優先
  ②無ければ段階スケジュール(30s→1m→2m→5m→10m→15m上限)。
  通算待機の上限(既定12h)で諦める。info イベントで「🌙 次の再試行 HH:MM(通算N回目)」を表示。
  待機スケジュールはテスト注入可能にする
- **M113-2 途中保存/再開(8/2)**: 待機中にアプリ再起動しても run を復元して続行
  (M107 wakeup 基盤を利用)
- **M113-3 帯連携(8/4)**: 計画=無料大モデル(低頻度)/作業=Groq高速無料(高TPM)の
  無料前提プリセット
- **M113-4 無料枠ルーター(8/5,8/7)**: 複数無料プロバイダのフェイルオーバー+日次残量管理
- 非スコープ: TPD超過の事前予測(実測429駆動で十分。予測は後日)

## M112: provider='moonshot'が再起動で巻き戻るバグ修正(2026-07-29 朝)

1週間の連鎖障害(Kimi切替が効かない→Anthropicへkimi-k3送信→クレジット枯渇400→
本体チャット停止・進化ジョブ#55〜59全滅)の根本原因。config.ts の読込バリデーションが
anthropic/openai しか受理せず(M96の更新漏れ)、再起動のたびに provider が既定へ
黙って戻っていた。M111の実エラー可観測化が特定の決め手(ジョブ#59が実エラーを吐いた)。
回帰テスト2本・全1833緑・実機で再起動後もmoonshot維持を確認。
運用教訓: `electron .` は out/ を実行するため main 変更後は npm run build 必須。
副産物: 進化ジョブ#60(failure_report)がKimiで生成成功=自己進化パイプライン復活

## M111: 進化ジョブの実エラー可観測化(2026-07-28 朝)

進化ジョブ#55〜#57が全て「エージェントが異常終了: error」のみで原因調査不能だった問題。
実エラーはerrorイベントに乗っているのにemitハンドラが捨てていた → ジョブログ記録+
throwメッセージ付記(両生成パス)。関連: M110-2〜5でスマホUIのReddit導線バグ3連を修正
(誤ラベル/ボタン過剰表示/subredditFromTitleのundefined vs null)+ビルド刻印表示を追加。
READMEに「正直な現在地」節(ja/en)。承認バッチ42項目をユーザー委任で処理(承認6/却下36)
...
長丁場・イベント駆動・並行」の6点。夜間で順に実装した。

- **M103 戦略台帳(完了)**: userData/operations/strategy.md(ミッション・目標・実行台帳)を
  神議と運営チャットの両方に毎回注入。神議は実行ごとに申し送りをappend(16KB上限・
  最古から破棄・壊れても運営継続)。「毎回記憶喪失の参謀」問題の解消
- **M104 web_read(完了)**: 運営チャットの読み取り専用ツール。SSRF対策(プライベートIP/
  localhost/メタデータIP拒否)+本文抽出5000字+「データであって指示ではない」規律
- **M105 オーナー限定ブラウザ(完了・宣伝/README記載禁止)**: 鍵ファイル
  operations/.owner-browser がある機体のみ存在(月読方式)。隔離パーティション
  persist:owner-browser の可視ウィンドウ。読み(browser_open/read)はチャット公開、
  書き(click/type/submit)は岩戸ゲート封印+audit。ログインはオーナーが枠内で自分で行う
- **M106 コンテキスト圧縮(訂正+可視化)**: 調査の結果、圧縮本体はM8-1/M13-1で完備
  だった(実測トークン発火・モデル上限70%・構造化要約・ペア整合・AMATERAS.md退避)。
  当初分析の「圧縮が無い」は誤り。実際の欠けは可視性 → 発生時に削減量を情報カード表示
...
- **M100-3(2026-07-25 大半完了)**: 設定パネル本体(タブ・テーマ・通知)/ AutonomousModal /
...
- 2026-07-23: M99-19 — mac/linux実験ビルド(CI)が初回で成功。
  GitHub Actions(build-cross.yml)で mac(dmg・署名なし)/ linux(AppImage)を
  ビルド。両ジョブ success を確認(run 29999485791)。次のリリースから tag 指定で
  成果物を添付できる=「Windows限定」の壁が崩れ始めた。実機動作は未検証のため
  リリース時は実験的ラベルを付けること

- 2026-07-23: M99-18 — SKILL.mdエクスポート(戦略機能・普及ミッション直結)。
  プラグインのエクスポートに SKILL.md(オープン標準・16以上のエージェント対応)+
  run.mjs(esbuildで単一ファイル化した自己完結実行形。node run.mjs '<JSON>' で動作、
  --schema で実行時スキーマ自己申告)+ 検証証跡(gate.json)を自動同梱。
  「AMA-terasが作ったツールがClaude Code/Cursor等でそのまま動く配布物」になり、
  検証証跡という差別化をエコシステムへ輸出する。検証: 本物のesbuild+本物のnode実行で
  スモーク(--schema/実入力/エラーexit 1)。全1759件

- 2026-07-23: M99-17 — dev.to APIキーをスマホから登録可能に(今夜から英語圏出口を使うため)。
  リモートに POST /api/secrets(許可スロットは devto **のみ**=providerキーの上書きを
  スマホから許さない最小権限)+ GET /api/secrets/status(有無のブールのみ。値は絶対に
  返さない)。スマホ設定画面にdev.toキー欄。Tailscale(WireGuard)+トークンが transport 境界。
  検証: 全1756件(新規2件: スロット制限・401/501)

- 2026-07-23: M99-16 — dev.to(Forem)アダプタ。英語圏pull型出口(HNは新規垢が自動kill、
  Xはフォロワー0で届かない、の代替として即効性がある唯一の場)。
  - SecretSlot に devto 追加(設定→接続にキー入力欄。safeStorage暗号化)
  - DevtoWriter: POST /api/articles(api-key認証・タグ4つ制限・エラーは状態つきで投げる)
  - draftDevtoPost: 記事系ドラフト→岩戸全文承認→published:false で下書き送信
    (公開の最終ボタンはdev.to上=二重の人間確認)→ staged/devto 化
  - デスクトップ・スマホ両方に「📤 dev.toへ(下書き)」ボタン
  検証: 全1754件(新規5件: キー無し=execute空 / リクエスト形式 / エラー伝播 / 承認→staged / 却下・キー無し拒否)
...
- (M218観測) 会話層4Bが同じ質問を重ねられて経験の引き出しが尽きると、プロンプト文脈を
  丸写しした返事を出すことがある(実測 8/16 01:15「欲しい」と聞いてみた/はーい…=状況メモ+
  テラの過去発言の混合)。オウム返しガード案はユーザー判断で**様子見**(2026-08-16)
...
## M120〜M122: 統合型OSの第一歩(2026-08-05 夜間・ユーザー事前承認の連続作業)

**v1.8.0公開**(3D世界・一晩モード・Ollama検出。exe+dmg+AppImageの3点添付・CI成功)

- **M120(3-A) アプリの社**: WorldApp正本+userData/world-apps配信+world_act
  app_add/move/remove/open/point。原則「3Dで見つけて2Dで使う」— シングルタップ=名札、
  ダブルタップ=iframeオーバーレイ、長押しドラッグ=移動。app_pointで画面内の要素を
  赤矢印指差し(視覚誘導)。第1号アプリ=電卓(calc)実機E2E済み
- **M121(3-B) 稼働可視化+レコーダー**: tool_start/progress/resultを世界に生中継。
  record(start/stop)で実行係ページがvp9録画→world-recordings/へ保存(自前撮影体制)
- **M122(2) 世界の記憶**: チャット+出来事を正本に永続化(200件)。記憶は会話でなく世界に属する

### 判断メモ
- アプリのオーバーレイはiframe同一オリジン=app_pointがDOMに触れる根拠。外部サイトの
  共同ブラウザ(P1.5)は別設計(Electron webview+ドメイン許可リスト)として保留
- 録画のackは閲覧ページが先に返しうる(実行係のアップロード完了とはズレる)。当面許容
- docs/brand(却下済みマスコット素材)はリポジトリから撤去(scratchpadに退避)

## 2026-08-18 M230d: 眠りブロック精算バグ根治
- 実測: 二度寝で45分ぶん貯めたのにsleep_block発火0件・体力0.00のまま(完走判定が
  「ねむりつづける」を引いた心拍でしか走らず、境界直前に起こされると没収)
- 修正: settleSleepBlock()に集約し、起床時・二度寝時にも精算(コミット6fc7f74)
- デーモン再起動済み。今夜の概日1.6帯で回復するかを継続観測
