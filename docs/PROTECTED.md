...
8. **岩戸ゲート(M32-2追加)**: Protocol AMANO-iwate の承認ゲート。外部への発信
   (投稿・コメント・ラベル・マージ)はすべてこのゲートの承認フロー
   (何を・どこへ・全文プレビュー)を通る。承認なし実行をコードレベルで
   不可能にする封印実装そのものを保護する
...
| `src/main/operations/protocol.ts` | 8. 岩戸ゲート(外部発信の承認ゲート) |
