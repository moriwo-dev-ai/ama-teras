...
すべて stdio / npx 起動型(MyCodex v1の対応範囲)。Settings → MCPサーバー から追加するか、
`%APPDATA%\mycodex\mcp.json` に直接書く。

## まずこの3つ(コード・ドキュメント・ブラウザで日常の8割をカバー)

### 1. Playwright(ブラウザ操作)— 一番のおすすめ

**MyCodexの弱点だった「UI確認」を今すぐ部分的に解消できる。**
このサーバーはスクリーンショットではなく**構造化テキスト(アクセシビリティスナップショット)**で
ページ内容を返すため、画像入力に未対応の現MyCodexでもそのまま使える。
...
MyCodexには組み込みのファイルツールがあるため通常は不要。
...
- GitHub: 「mycodex リポジトリの直近のコミット5件を要約して」
