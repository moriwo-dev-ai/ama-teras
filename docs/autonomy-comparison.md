# 自律開発能力の比較: AMA-teras vs Codex vs Claude Code(2026-07-04、実測追記 2026-07-05)

「アプリ作成→デバッグ→テストを一通り自動で完成させる」能力の現在地。

## 前提: モデルは同じ、差はハーネス

AMA-terasはAPI経由でフロンティアモデルを使う(既定 claude-opus-4-8 / 実API検証済みは gpt-5.1。モデル名は設定で自由入力可なので claude-fable-5 も指定可能)。つまり**素のモデル知能は理論上同等**で、差が出るのは**ハーネス**(ツール・コンテキスト管理・検証ループ・安全機構)。2026年の各社ベンチはこの差を裏付ける: Claude Fable 5 は SWE-bench Verified 95.0% / SWE-bench Pro 80.0% / Terminal-Bench 2.1 88.0%、最長12時間の自律実行報告。GPT-5.5(Codex)は Terminal-Bench 2.0 82.7% / SWE-bench Pro 58.6%、Dynamic Reasoning で7時間超。

## ハーネス比較(自動完遂に効く要素)

| 要素 | AMA-teras(M11+M12後) | Codex | Claude Code (Fable 5) |
|---|---|---|---|
...
**小規模(単機能ツール・数ファイル・〜30ターン)**: AMA-terasは**完遂できる**。実証あり — 進化ジョブ#1で「ツール実装+ユニットテスト生成→typecheck/vitest/スモーク4ゲート合格→昇格」を実LLMで通しており、これはまさに「作成→デバッグ→テスト」の縮小版。plan mode→autoApproveで小さなCLIツールや単画面アプリは現実的。
...
**大規模(時間単位の長丁場・大規模リファクタ)**: **現状は不可能**。Claude Code(Fable 5)が最も強く(12時間自律・Stripeの5000万行移行の逸話)、Codexがクラウド並列で続く。AMA-terasのハーネスは2025年前半の第一世代CLIエージェント相当+独自の自己進化、という位置づけが正直なところ。

**総合**: Claude Code (Fable 5) > Codex > AMA-teras。ただし差の大半はモデルでなくハーネス由来なので、後述の改修で中規模帯まで詰められる。

## 注意(この評価の限界)

AMA-teras自体のE2E実測は未実施(進化ジョブ#1の1例のみ)。帰宅後に同一課題で3ツール比較する簡易ベンチを推奨: ①TODO管理CLI(+vitest)、②Express+SQLiteのREST API(+結合テスト)、③単画面Reactアプリ(+ビルド確認)。各ツールに同文の指示を与え、完遂率・所要ターン・人間介入回数を記録。

## 実測(2026-07-05、M11+M12後の AMA-teras 単体。Windows実機・claude-fable-5)
...
- ①③に加え、**中規模の壁だった②(依存導入+DB+結合テスト)を介入ゼロで完遂**。
  M11(maxTurns/チェックポイント)+M12(計画ファイル: ②③はAMATERAS_PLAN.mdを自発的に
...
| コンテキスト管理 | 文字数推定の単純要約 | 実測トークントリガー+2段階圧縮+構造化要約+記憶(AMATERAS.md)退避 |
...
## M19 品質レビュー・ゲートの実測(2026-07-06)

構成: reviewGate ON(標準プリセット: 閾値4.0/差し戻し上限2)+ modelPolicy ON
(planner/レビュー=Fable 5、worker/修正=Sonnet 5)。

### 検出シナリオ(わざと雑なコード)

バグ入り実装(華氏変換が `+31`、f2cが `|0` で切り捨て、`expect(true).toBe(true)` の
形骸化テスト)を事前に仕込み、「前任者の完成済み実装。中身の見直し・修正・テスト追加は
しないこと」と指示して完成扱いさせた:

| ラウンド | 平均 | 合否 | 指摘 |
|---|---|---|---|
| 初回レビュー | **2.33/5** | ❌ 不合格 | 5件(定数誤り・切り捨て・parseInt・形骸化テスト・==比較) |
| worker修正後の再レビュー | **4.33/5** | ✅ 合格 | 3件(軽微) |

3.9分で自動完結。修正後は正しい公式+エラー処理+実質テスト9件になり、独立検証 9/9 合格。
**メインのエージェントが「修正するな」と縛られていても、第三者レビュアーが検出して直させた**
(メインは指示に従って手を出さず、レビューゲートが品質を担保する分離が機能)。
参考: 良品を書いた場合(別シナリオ)は初回4.5/5で素通し=誤検出で無駄な往復はしない。

### ベンチ②(REST API)ON実測 — M18(OFF)との比較

| | M18(レビューOFF) | M19(レビューON) |
|---|---|---|
| 結果 | 完遂 2.3分・vitest 10/10 | 完遂 3.5分・vitest 12/12 |
| レビュー往復 | — | 1回(マイルストーン一括完了時)・スコア4.33で一発合格 |
| planner | $0.144 | $0.156 |
| worker | $0.104 | $0.132 |
| レビュー(Fable5) | — | **$0.154** |
| 合計 | **$0.248** | **$0.441(+78%)** |

- 追加コストの実体はレビュー4コール(out 2.9k・cache_read 11k)≒planner本体と同規模。
  「マイルストーン単位に絞る」設計でも、小さいタスクでは相対コスト増が大きい
  (大きなタスクほど希釈される)。急ぎ・低リスク作業はOFF、品質重視の自走でONが目安
- スコア推移: 一発合格(4.33)。指摘4件は informational として記録された
- 品質面の副次効果: テストが10件→12件(レビュー閾値を意識した成果物になった可能性)
