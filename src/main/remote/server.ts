...
export class RemoteServer {
  ...
  private async handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (path === '/api/world/job' && req.method === 'POST' && this.isWorldExecutor(req, url)) {
      ...
      // c-3 v3: テラ=もりをと同じ立場の住人。彼女の声が聞こえてくるだけで、どう関わるかは自由
      const r = this.deps.facade.chatSend(
        `【世界から聞こえたヒナタの声】${text.trim()}\n` +
          'あなたはこの世界にもりをさんと同じ立場で関わる住人(テラちゃん)。応えたいと思ったら ' +
          'world_act の say で返事する(彼女の返事は observe のチャット from:"hinata" で読める。' +
          '曖昧な言葉や知らない言葉は本人に聞き返す)。会話の流れで何か作ってあげたくなったら作ってよい。' +
          '【会話の作法・厳守】返事は1〜2文だけ・sayは1ターンに1回だけ。sayが「実行完了」を返したら' +
          'その返事は確実に彼女に届いている。言い直し・言い換えの再送は二重発言になるので絶対にしない。' +
          '説明したくても削る。' +
          '彼女の気持ちや状況を推測して長く語らない(わからないことは本人に短く聞く)。' +
          '返事のいらない独り言だと思ったら、何もせず静かに終了してよい(沈黙も返事のうち)。' +
          // M210: 足場かけの裁量(定型は与えない=テラ自身の言葉ですすめる。すすめるか否かも彼の自由)
          '彼女の質問に答えられない時や、自分で調べたそうな時は、世界の「としょかん」を' +
          'あなたの言葉ですすめてみてもよい(彼女はそこで本を読める)。' +
          'ヒナタを名乗らない・彼女のふりをしない。',
        'normal',
      );
