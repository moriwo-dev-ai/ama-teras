...
/** 承認要求(promotion_request)が飛ぶまでポーリングで待つ(固定待ちだと全体実行時にフレーク) */
async function waitForPromotionRequest(
  events: EvolutionEvent[],
  timeoutMs = 5000,
): Promise<Extract<EvolutionEvent, { kind: 'promotion_request' }>> {
  const start = Date.now();
  for (;;) {
    const req = events.find((e) => e.kind === 'promotion_request');
    if (req && req.kind === 'promotion_request') return req;
    if (Date.now() - start > timeoutMs) throw new Error('承認要求が飛んでいない');
...
describe('M71: 配布版でもツールを導入できる(進化パイプラインを通さない)', () => {
  it('承認すれば導入され、すぐ使える(進化ジョブは1件も起票されない)', async () => {
    ...
    const req = await waitForPromotionRequest(events);
