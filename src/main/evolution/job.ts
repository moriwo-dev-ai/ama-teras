...
- テスト方針: 中心ロジックは**純関数として named export** し、テストは \`import { fn } from './<name>'\`
  で直接呼んで入出力を確かめる(execute も import して {content} を検証してよい)。
  **プラグインローダ経由(loadPluginFile・mkdtemp でプラグインを読み込む等)の統合テストは書かない** —
  葉ツールに不要で過剰。手本のテストの流儀をそのまま真似ればよい
- child_process やネットワークアクセスを使う場合は warnings に自己申告すること

スコープ(過剰探索の禁止 — 破ると予算超過で打ち切られる。実測で無駄の主因):
- このツールは他から切り離された「葉」。**アプリ内部(プラグインローダ・src/main/ipc・
  package.json・vitest設定など)を調べる必要は一切ない**。上に手本の既存プラグインが貼ってある。
  read_file / grep で**手本以外のアプリ本体を探索しないこと**(そこに答えは無い・時間と費用の無駄)
- 手本に倣って一気に書き、下の自己検証2コマンドが通ったら**すぐ完了する**。
  edit→test を3回以上繰り返しそうになったら、実装かテストが過剰になっている合図。純ロジックへ単純化せよ
