...
describe('【不変条件2】--smoke-boot のロック非取得+userData隔離(ソース・トリップワイヤ)', () => {
  ...
  it('--smoke-boot は MYCODEX_SMOKE 配下でのみ有効(=単一インスタンスロックのスキップに乗る)', () => {
  ...
  it('gates の renderer/core スモークは --smoke-boot + MYCODEX_SMOKE で起動する(ソース固定)', () => {
    ...
    expect(src).toContain("env: { MYCODEX_SMOKE: '1' }");
  });
});
