...
export async function registerIpcHandlers(
    ...
    try {
      service.chatSend(
        `[世界チャット] ユーザーが世界(共有3D空間)であなたに話しかけた:「${text}」\n` +
          '進め方(M130: 沈黙はユーザーの不安になる=返事ファースト):\n' +
          '①最初のツール呼び出しは world_act の say — 返事と「どう進めるか」を1〜2文で先に伝える' +
          '(例:「了解!○○だね。まず△△して、次に□□するね」)。作業はその後\n' +
          '②作業が長引く時・問題が起きた時は、節目ごとに say で状況と次の方針を短く共有する' +
          '(例:「セレクタがずれてたみたい。付け直してもう一回試すね」)\n' +
          '③完成・完了時も say で締める。世界のユーザーには say しか見えない\n' +
          'world_observe で状況(openApp=ユーザーが開いているアプリ)を見てから動くこと。' +
          '身振り(motion)や spawn での図解など「世界で見せる」表現を優先する。',
        'normal',
      );
    } catch (err) {
  ...
  ipcMain.handle(IpcChannels.operationsGodRun, (_e, godId: unknown) => {
    assertString(godId, 'godId');
    return operations.runGodNow(godId);
  });
