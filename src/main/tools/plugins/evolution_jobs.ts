...
const plugin: ToolPlugin = {
  ...
  async execute(input: unknown, ctx: ToolContext): Promise<ToolResult> {
    ...
    if (jobs.length === 0) {
      // M89: 配布版で「ジョブはありません」とだけ答えると、エージェントは「申請が消えた」
      // 「呼び方が悪い」と誤診して迷走する。配布版には**ジョブという概念自体が無い**
      if (ctx.packaged === true) {
        return {
          content:
            '配布版には進化ジョブがありません(そもそも起票されない)。配布版では新規ツールの**生成**は無効で、' +
            'コミュニティレジストリや手元のファイルからの**既存プラグインの導入**だけができます。導入は検査→承認→配置で' +
            'その場で完結し、ジョブとしては残りません。導入済みのツールは tools 一覧に出ます。' +
            '新規にコードを書き起こすツールが必要なら、開発版(git clone → npm install → npm start)が要ります。',
        };
      }
      return {
        content:
