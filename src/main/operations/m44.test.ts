...
describe('M45: 単発ボタン経路も「投稿済み」にする(一括だけが処理していた)', () => {
  it('GitHub Release: 出せたら投稿済み。二度目は実行しない', async () => {
    const drafts = new DraftStore(join(dir, 'operations'));
    const id = drafts.add([{ kind: 'release-note', title: 'v1.1.0', body: '## 変更\n- あれこれ' }])[0]!.id;
    const manager = new OperationsManager({
      userDataDir: dir,
      ...
      approvalPrompt: () => Promise.resolve(true),
      bandProvider: () => 'キー未設定',
      ghRunner: async () => 'https://github.com/o/r/releases/tag/v1.1.0',
    });
    await manager.status();

    const r = await manager.requestRelease(id, 'o/r', 'v1.1.0');
    expect(r.ok).toBe(true);
    // M57: GitHub Release は draft で作られる = まだ公開されていない。staged(公開待ち)
    expect(manager.listDrafts().find((d) => d.id === id)?.status).toBe('staged');

    const again = await manager.requestRelease(id, 'o/r', 'v1.1.0');
    expect(again.ok).toBe(false);
    expect(again.detail).toContain('すでに出している');
  });
});

describe('M46: リリースのバージョンを自動採番する(前回の版を人間が覚えない)', () => {
  it('次の版: patch/minor/major を機械的に出す。解釈できないタグは触らない', () => {
    expect(nextVersion('v1.0.0', 'patch')).toBe('v1.0.1');
    expect(nextVersion('v1.9.3', 'minor')).toBe('v1.10.0');
    expect(nextVersion('v1.9.3', 'major')).toBe('v2.0.0');
    expect(nextVersion('1.0.0', 'patch')).toBe('v1.0.1'); // v無しも受ける
    expect(nextVersion('nightly', 'patch')).toBeNull();
    expect(nextVersion(null, 'patch')).toBeNull(); // 初回=UIがアプリ版を使う
  });

  it('最新リリースを見て候補を返し、package.json との食い違いを知らせる', async () => {
    const manager = new OperationsManager({
      userDataDir: dir,
      getConfig: () => ({ operations: { enabled: true, repos: ['o/r'], zennSlugs: [] } }) as unknown as AppConfig,
      audit: () => {},
      ...
      ghRunner: async (args: string[]) =>
        args[0] === 'release' && args[1] === 'view' ? JSON.stringify({ tagName: 'v1.2.0' }) : '',
    });
    await manager.status();

    const info = await manager.releaseInfo('o/r');
    expect(info.latestTag).toBe('v1.2.0');
    expect(info.suggestions).toEqual({ patch: 'v1.2.1', minor: 'v1.3.0', major: 'v2.0.0' });
    // アプリ版(1.0.0)が直近リリース(v1.2.0)より古い = package.json の更新忘れ
    expect(info.mismatch).toBe(true);
    expect(sameVersion('v1.2.0', '1.2.0')).toBe(true);
  });

  it('リリースが1件も無ければ候補なし(UIはアプリの版を初期値にする)', async () => {
    const manager = new OperationsManager({
      userDataDir: dir,
      getConfig: () => ({ operations: { enabled: true, repos: ['o/r'], zennSlugs: [] } }) as unknown as AppConfig,
      audit: () => {},
