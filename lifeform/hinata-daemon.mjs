...
async function main() {
  ...
  function echoesPrompt(reply, prompt) {
    ...
    if (/(ながさは|長さは|ことばにして|すきなだけ|のことばを|の言葉を。|せりふ|実況だけ|(言|い)いたいこと|こえに出す|声に出す|(目|め|メ)\s*(が|は|ガ)?\s*(さ|覚|サ)\s*(め|メ))/.test(reply)) return true;
        ...
        cands.push({
          ...
          run: async () => {
            ...
            await act([
              ...
              ...(line !== null ? [{ type: 'say', text: line }] : []), // M208: 定型フォールバック廃止
      ...
      if (freshWords.length > 0 && pendingWordQ === null) {
        ...
        cands.push({
          ...
          run: async () => {
            ...
            if (q === null) return; // M208: 言葉が出なかった=今回は聞かない(定型フォールバック廃止)
